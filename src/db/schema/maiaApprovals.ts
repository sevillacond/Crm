import { pgTable, varchar, timestamp, text, jsonb, unique, index } from 'drizzle-orm/pg-core';
import { instancesTable } from './instances.ts';

export const maiaApprovalRequestsTable = pgTable(
  'maia_approval_requests',
  {
    ...
    autonomyLevel: varchar('autonomy_level', { length: 16 }),
    riskLevel: varchar('risk_level', { length: 32 }),
    toolPolicySnapshot: jsonb('tool_policy_snapshot'),
  ...
  (table) => ({
    uqApprovalInstance: unique('uq_maia_approvals_id_instance').on(table.id, table.instanceId),
    instanceIdx: index('idx_maia_approvals_instance').on(table.instanceId),
    statusIdx: index('idx_maia_approvals_status').on(table.status),
    createdAtIdx: index('idx_maia_approvals_created_at').on(table.createdAt)
  })
);
