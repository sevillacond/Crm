import { pgTable, text, timestamp, varchar, integer, numeric, boolean, jsonb, unique } from 'drizzle-orm/pg-core';
import { instancesTable } from './instances.ts';

export const planosTable = pgTable(
  'planos',
  {
  ...
  (table) => ({
    uqPlanoInstance: unique('uq_planos_id_instance').on(table.id, table.instanceId)
  })
);
